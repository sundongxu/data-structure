...
public:
  ...
  AdjListDirNetwork(ElemType es[], int vertexNum = DEFAULT_SIZE, WeightType infinit = (WeightType)DEFAULT_INFINITY);
  // 构造顶点数据为es[]，顶点个数为vertexNum，infinit表示无穷大，边数为0的有向网
  AdjListDirNetwork(int vertexNum = DEFAULT_SIZE, WeightType inifinit = (WeightType)DEFAULT_INFINITY);
  ...
  AdjListDirNetwork<ElemType, WeightType> &operator=(const AdjListDirNetwork<ElemType, WeightType> &copy); // 赋值运算符重载
};
...
template <class ElemType, class WeightType>
bool AdjListDirNetwork<ElemType, WeightType>::IsVertex(int v) const
{
  // 操作结果：判断顶点v序号范围是否合法
  if (v < 0 || v >= vexNum)
  {
...
template <class ElemType, class WeightType>
bool AdjListDirNetwork<ElemType, WeightType>::IsEdge(int v1, int v2) const
{
  // 操作结果：判断边(v1, v2)是否存在
...
template <class ElemType, class WeightType>
int AdjListDirNetwork<ElemType, WeightType>::GetElem(int v, ElemType &e) const
{
  // 操作结果：返回顶点v对应的元素值
...
template <class ElemType, class WeightType>
int AdjListDirNetwork<ElemType, WeightType>::SetElem(int v, const ElemType &e)
{
  // 操作结果：设置顶点元素值
  // 判断v是否在合理顶点范围内，1<=v<=vexNum
  if (IsVertex(v))
  ...
  else if (IsVertex(v1) && IsVertex(v2))
  {
    ...
    else
    {
      ...
      if (indexV2 == vexTable[v1].adjLink->Length() + 1)
      {
        ...
        AdjListNetworkEdge<WeightType> tmpEdge(v2, w);
        vexTable[v1].adjLink->Insert(indexV2, tmpEdge); // 将v2插入到v1邻接表的末尾
      }
    }
  }
  edgeNum++;
...
void AdjListDirNetwork<ElemType, WeightType>::SetWeight(int v1, int v2, WeightType w)
{
  ...
  if (IsEdge(v1, v2))
  {
    ...
    if (curPosV2 <= vexTable[v1].adjLink->Length())
    {
      // 存在边<v1,v2>
      vexTable[v1].adjLink->GetElem(curPosV2, tmpEdge); // 取出边
      tmpEdge.weight = w;                               // 修改<v1,v2>权值
      vexTable[v1].adjLink->SetElem(curPosV2, tmpEdge); // 设置边
    }
  }
}

template <class ElemType, class WeightType>
int AdjListDirNetwork<ElemType, WeightType>::GetTag(int v, int &val) const
{
...
template <class ElemType, class WeightType>
int AdjListDirNetwork<ElemType, WeightType>::SetTag(int v, const int &val)
{
