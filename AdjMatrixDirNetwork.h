#ifndef ADJ_MATRIX_DIR_NETWORK_H
#define ADJ_MATRIX_DIR_NETWORK_H
...
public:
  ...
  AdjMatrixDirNetwork(ElemType es[], int vertexNum = DEFAULT_SIZE, WeightType infinit = (WeightType)DEFAULT_INFINITY);
  // 构造顶点数据为es[]，顶点个数为vertexNum，infinit表示无穷大，边数为0的有向网
  AdjMatrixDirNetwork(int vertexNum = DEFAULT_SIZE, WeightType inifinit = (WeightType)DEFAULT_INFINITY);
  ...
  AdjMatrixDirNetwork(const AdjMatrixDirNetwork<ElemType, WeightType> &copy);                                  // 复制构造函数
  AdjMatrixDirNetwork<ElemType, WeightType> &operator=(const AdjMatrixDirNetwork<ElemType, WeightType> &copy); // 赋值运算符重载
};
...
template <class ElemType, class WeightType>
bool AdjMatrixDirNetwork<ElemType, WeightType>::IsVertex(int v) const
{
  // 操作结果：判断顶点序号范围是否合法
  if (v < 0 || v >= vexNum)
  {
...
template <class ElemType, class WeightType>
bool AdjMatrixDirNetwork<ElemType, WeightType>::IsEdge(int v1, int v2) const
{
  // 操作结果：判断边（v1,v2）是否存在
...
template <class ElemType, class WeightType>
int AdjMatrixDirNetwork<ElemType, WeightType>::GetElem(int v, ElemType &e) const
{
  // 操作结果：返回顶点元素值
...
template <class ElemType, class WeightType>
int AdjMatrixDirNetwork<ElemType, WeightType>::SetElem(int v, const ElemType &e)
{
  // 操作结果：设置顶点元素值
  // 判断v是否在合理顶点范围内，1<=v<=vexNum
  if (IsVertex(v))
...
void AdjMatrixDirNetwork<ElemType, WeightType>::InsertEdge(int v1, int v2, WeightType w)
{
  ...
  {
    // v1与v2都存在
    Matrix[v1][v2] = w; // 邻接矩阵中指定元素（v1,v2）赋值为w
    edgeNum++;
  }
}
...
template <class ElemType, class WeightType>
int AdjMatrixDirNetwork<ElemType, WeightType>::GetTag(int v, int &val) const
{
...
template <class ElemType, class WeightType>
int AdjMatrixDirNetwork<ElemType, WeightType>::SetTag(int v, const int &val)
{
...
template <class ElemType, class WeightType>
void AdjMatrixDirNetwork<ElemType, WeightType>::SetWeight(int v1, int v2, const WeightType &w)
{
  // 操作结果：设置顶点v1和v2的边的权值
  if (IsEdge(v1, v2))
  {
...
#endif // ADJ_MATRIX_DIR_NETWORK_H